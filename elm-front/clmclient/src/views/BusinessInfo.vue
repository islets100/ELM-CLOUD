...
<script>
	export default {
		...
		created() {
			...
			//根据businessId查询商家信息
			this.$axios.get(`businesses/${this.businessId}`).then(response => {
				let ret = response.data;
				this.business = ret.result;
			}).catch(error => {
				console.error(error);
			});
		...
		methods: {
			listCart() {
				this.$axios.get(`carts/user/${this.user.userId}/business/${this.businessId}`).then(response => {
					let ret = response.data;
					let cartArr = ret.result;
					//遍历所有食品列表
			...
			updateCart(index, num) {
				this.$axios.put(`carts/user/${this.user.userId}/business/${this.businessId}/food/${this.foodArr[index].foodId}`, this.$qs.stringify({
			...
			removeCart(index) {
				this.$axios.delete(`carts/user/${this.user.userId}/business/${this.businessId}/food/${this.foodArr[index].foodId}`).then(response => {
					let ret = response.data;
					if (ret.code == 200) {
