...
<script>
	...
	export default {
		...
		created() {
			...
			//根据orderTypeId查询商家信息
			this.$axios.get(`businesses/order-type/${this.orderTypeId}`).then(response => {
				let ret = response.data;
				this.businessArr = ret.result;
				//判断是否登录
				if(this.user!=null){
					this.listCart();
				}
		...
		methods:{
			listCart(){
				this.$axios.get(`carts/user/${this.user.userId}`).then(response => {
					let ret = response.data;
