<template>
	<div class="wrapper">
		...
		<div class="order-info">
			...
			<p>{{deliveryaddress!=null?deliveryaddress.contactName:''}}{{deliveryaddress!=null?(deliveryaddress.contactSex==1?'先生':'女士'):''}} {{deliveryaddress!=null?deliveryaddress.contactTel:''}}</p>
		</div>
...
<script>
	export default {
		...
		async created() {
			...
			//查询当前商家
			let response = await this.$axios.get(`businesses/${this.businessId}`);
			this.business = response.data.result;
			
			//查询当前用户在购物车中的当前商家食品列表
			response = await this.$axios.get(`carts/user/${this.user.userId}/business/${this.businessId}`);
			let cartArr = response.data.result;
			//获取食品对象
			for(let i = 0; i< cartArr.length;++i){
				response = await this.$axios.get(`foods/${cartArr[i].foodId}`);
				cartArr[i]["food"]=response.data.result;
			}
