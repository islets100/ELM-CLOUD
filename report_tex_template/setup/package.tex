...
\usepackage[UTF8,fontset=fandol]{ctex}      % 支持中文显示（跨平台字体集）
...
\fi
\usepackage{float}      % 提供[H]选项，用于固定图片位置
\usepackage{placeins}   % 用于更好的浮动控制
\usepackage{seqsplit} % 导入长单词自动换行包
...
% TypeScript 语法高亮（listings 原生不包含）
